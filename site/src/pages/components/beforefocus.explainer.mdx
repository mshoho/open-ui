...
The `detail` property indicates why the element is about to receive focus.

> **Note:** The numeric values below are currently taken directly from Chromium's internal
> `FocusType` enum. The final API should use different values, WebIDL enum, or a dedicated
> property instead of `detail`. We most likely don't want to directly expose `0` when
> the focus is moved by an assistive technology (like a screen reader). For the screen reader
> focus movements we most likely want to send Forward/Backward value (depending on the relative
> position of a new element comparing to the current one).
